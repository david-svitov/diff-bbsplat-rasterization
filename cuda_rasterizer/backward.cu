...
float calc_angle(float* normal, float* direction){
    float normal_norm = sqrt(normal[0]*normal[0] + normal[1]*normal[1] + normal[2]*normal[2]);
    float direction_norm = sqrt(direction[0]*direction[0] + direction[1]*direction[1] + direction[2]*direction[2]);

    float cos_val = (normal[0]*direction[0] + normal[1]*direction[1] + normal[2]*direction[2]) / (normal_norm * direction_norm);
    return fabs(cos_val);
}
	...
	float* __restrict__ dL_dtexture_color)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			texture_offset = global_id * texture_pixels * 3;
			float sampled_color_array[3];
			biliniar_texture_sampler(sampled_color_array, s.x, s.y, texture_color + texture_offset, texture_size, 3);

			for (int ch = 0; ch < C; ch++)
			{
			    const float sh_color = collected_colors[ch * BLOCK_SIZE + j];
			    const float tex_color = sampled_color_array[ch];
				const float c = tex_color + sh_color;
