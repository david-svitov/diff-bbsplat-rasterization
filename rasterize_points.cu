	...
	const bool debug)
{
  ...
  if (texture_alpha.size(1) != texture_alpha.size(2)) {
    AT_ERROR("alpha texture should have square shape");
  }

  if (texture_color.size(2) != texture_color.size(3)) {
    AT_ERROR("color texture should have square shape");
  }

  if (texture_color.size(1) != 3) {
  ...
  const int texture_size = texture_alpha.size(1);

  CHECK_INPUT(background);
  CHECK_INPUT(means3D);
  CHECK_INPUT(colors);
  CHECK_INPUT(texture_alpha);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		texture_alpha.contiguous().data<float>(),
		texture_color.contiguous().data<float>(),
		texture_size,
	...
	const bool debug) 
{
  ...
  const int texture_size = texture_alpha.size(1);
  const int num_textures = texture_alpha.size(0);
  ...
  torch::Tensor dL_dtexture_alpha = torch::zeros({num_textures, texture_size, texture_size}, means3D.options());
  torch::Tensor dL_dtexture_color = torch::zeros({num_textures, 3, texture_size, texture_size}, means3D.options());
  ...
  {  
	  ...
	  dL_dtexture_alpha.contiguous().data<float>(),
	  dL_dtexture_color.contiguous().data<float>(),
