...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, depth, radii, impact, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
    ...
    @staticmethod
    def backward(ctx, grad_out_color, grad_radii, grad_impact, grad_depth):
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            try:
                grad_means2D, grad_colors_precomp, grad_texture_alpha, grad_textured_color, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations = _C.rasterize_gaussians_backward(*args)
            except Exception as ex:
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, texture_alpha, texture_color,
